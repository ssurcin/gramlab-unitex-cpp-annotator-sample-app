...
namespace unitexcpp
{
	namespace annotation
	{
		...
		LanguageArea LanguageArea::createLanguageArea(CAS& cas, size_t begin, size_t end, UnicodeString& language)
		{
			FSIndexRepository& indexRep = cas.getIndexRepository();
			AnnotationFS annotation = cas.createAnnotation(tLanguageArea, begin, end);
			if (!annotation.isValid())
				throw UnitexException("Unable to create LanguageArea");
			annotation.setStringValue(fLanguage, language);
			indexRep.addFS(annotation);

			return LanguageArea(annotation);
		}
